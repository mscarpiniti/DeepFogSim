% Defining and setting all the 65 global variables for Topology #6
%
% Topology #6 describes a network with M = 5 tiers and Q = 31 Fog nodes, 
...
cm_vec = [0.045, 0.1, 0.3, 0.5, 0.71, 0.7, 0.7, 0.9, 0.9];
...
M = 5; % Positive integer-valued number of tiers of the considered Fog 
       % platform. It must be L >= M
      
pertiernodes_vec = [16, 8, 4, 2 ,1]; 
...
A_mtx = [zeros(1,16), 1, zeros(1,14); 
         zeros(1,16), 1, zeros(1,14); 
         zeros(1,17), 1, zeros(1,13); 
         zeros(1,17), 1, zeros(1,13);  % Fog topolog#6;(k=2, M=5) tree topology
         zeros(1,18), 1, zeros(1,12); 
         zeros(1,18), 1, zeros(1,12); 
         zeros(1,19), 1, zeros(1,11); 
         zeros(1,19), 1, zeros(1,11); 
         zeros(1,20), 1, zeros(1,10); 
         zeros(1,20), 1, zeros(1,10); 
         zeros(1,21), 1, zeros(1,9); 
         zeros(1,21), 1, zeros(1,9); 
         zeros(1,22), 1, zeros(1,8); 
         zeros(1,22), 1, zeros(1,8); 
         zeros(1,23), 1, zeros(1,7); 
         zeros(1,23), 1, zeros(1,7); 
         zeros(1,24), 1, zeros(1,6); 
         zeros(1,24), 1, zeros(1,6); 
         zeros(1,25), 1, zeros(1,5); 
         zeros(1,25), 1, zeros(1,5); 
         zeros(1,26), 1, zeros(1,4); 
         zeros(1,26), 1, zeros(1,4); 
         zeros(1,27), 1, zeros(1,3); 
         zeros(1,27), 1, zeros(1,3); 
         zeros(1,28), 1, zeros(1,2); 
         zeros(1,28), 1, zeros(1,2);
         zeros(1,29), 1, 0; 
         zeros(1,29), 1, 0; 
         zeros(1,30), 1; 
...
L2Tmapping_vec = [2, 1, 2, 2, 2]; %Best uniform-like L2T mapping
% The [2 2 2 2 1] and [1 2 2 2 2] are no so good 
...
T_exit_vec = [T_s*ones(1, M-1), (0.85*0.435)];
...
P_IDLE_NET_RX_vec = (10^-7)*[zeros(1, pertiernodes_vec(1)), 0.60*ones(1, sum(pertiernodes_vec)-pertiernodes_vec(1))]; 
...
Zeta_NET_RX_vec = [zeros(1, pertiernodes_vec(1)), ...
    2.15*ones(1, sum(pertiernodes_vec)-pertiernodes_vec(1))]; 
...
a_max_backup = 2.5e-6;
% (1.3e-5 - 1.4e-5) is THE BEST RANGE at V_0 = 6 Mb
